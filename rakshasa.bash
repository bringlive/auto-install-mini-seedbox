...
# rTorrent / libtorrent versions (built from source)
RTVERSION="rakshasa-git"
LIBTORRENTVERSION="rakshasa-git"
...
############################################
# Build rTorrent + libtorrent from source
############################################
function BUILD_RTORRENT_FROM_SOURCE {

echo "${CYAN}Building libtorrent + rTorrent from Rakshasa Git${NORMAL}"
...
RemainAfterExit=yes
User=$RTORRENT_USER
ExecStart=/usr/bin/tmux -2 new-session -d -s session-rtorrent /usr/local/bin/rtorrent
