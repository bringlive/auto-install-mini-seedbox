...
function BUILD_LIBTORRENT_FROM_SOURCE {
	echo "${CYAN}Building libtorrent from source${NORMAL}"
	...
	# Extract version directly from source
	LIBTORRENTVERSION=$(grep -m1 "^AC_INIT" configure.ac | sed 's/.*\[\(.*\)\].*/\1/')

	cd /tmp
	rm -rf libtorrent

	echo "${GREEN}libtorrent build complete (${LIBTORRENTVERSION})${NORMAL}"
}

function BUILD_RTORRENT_FROM_SOURCE {
	echo "${CYAN}Building rtorrent from source${NORMAL}"
	...
	# Overwrite RTVERSION with the real built version
	RTVERSION=$(rtorrent --version 2>/dev/null | head -n1 | awk '{print $2}')
...
function INSTALL_APACHE {
	...
	apt-get install -yqq apache2 libapache2-mod-php php-cli php-curl php-mbstring php-xml php-json
	CHECKLASTRC
...
function INSTALL_NGINX {
	echo "${CYAN}Installing dependencies${NORMAL}"
	apt-get install -yqq nginx php-fpm php-cli php-curl php-mbstring php-xml php-json
	CHECKLASTRC
...
function INSTALL_LIGHTTPD {
	echo "${CYAN}Installing dependencies${NORMAL}"
	apt-get install -yqq lighttpd php-fpm php-cgi php-curl php-mbstring php-xml php-json
	CHECKLASTRC
}
